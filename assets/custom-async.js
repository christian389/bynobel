...
---------------------------------------------  */
/*global document, createElementWithClass, Validator, window, append_url, Cookies, Shopify, BVSelect, wrap, create_slider, openPopup, setTimeout, semanticTabs, toggle_dropdowns_simple, noUiSlider, Masonry, simplyCountdown, Fancybox, CountUp, new_css, close_dropdown, enquire,navigator, Promise, IntersectionObserver, Event , console, FormData, DOMParser, fetch, CustomEvent, yall, routes, randomize, handleCookieAccept, handleCookieDecline, whatInput, ImageCompare, sal, newsletter_popup_testmode, isMobile, localStorage, customDropHeight, countPrefixSuffix, general, Datepicker */
/* //jshint esversion: 6 */
...
var a_css_async = window.filepaths['a_css_async'],
	a_css_announcement = window.filepaths['a_css_announcement'],
	a_css_collection = window.filepaths['a_css_collection'],
	...
	a_css_filters = window.filepaths['a_css_filters'],
	...
	a_js_countup = window.filepaths['a_js_countup'],
...
var navAsync = function() {
	...
	if (nav_id) {
		...
		if (nd !== undefined) {
			ni = document.querySelectorAll('#nav > ul:first-child > li > a > img');
		}
		if (ni !== undefined) {
		...
		if (nav_top_id) {
			nav_top_clone = nav_top_id.querySelectorAll('ul[data-type]')[0];
			if (nav_top_clone) {
				nav_top_cloned = nav_top_clone.cloneNode(true);
		...
		if (nav_user_id) {
			nav_user_clone = nav_user_id.querySelectorAll('ul[data-type]')[0];
			if (nav_user_clone) {
				nav_user_cloned = nav_user_clone.cloneNode(true);
		...
		nav_gallery = nav_id.getElementsByClassName('recently-viewed')[0];
		if (nav_gallery) {
			nav_gallery_cloned = nav_gallery.cloneNode(true);
		...
		mobile_nav_header = createElementWithClass('div', 'header');
		...
		aria_controls_nav = document.querySelectorAll('a[aria-controls="nav"]');
		// #nav burger
...
function data_show_me(el) {
	...
	if (!isMobile && el.nextElementSibling.hasAttribute('data-element') && el.nextElementSibling.getAttribute('data-element') === el.getAttribute('data-enable')) {
		el.addEventListener('mouseenter', function (e) {
			el.nextElementSibling.classList.remove('hidden');
		});
		Array.from(document.querySelectorAll('[data-element="' + el.getAttribute('data-enable') + '"]')).forEach(function (em) {
			em.addEventListener('mouseleave', function (e) {
				el.classList.remove('hidden');
				em.classList.add('hidden');
			});
		});
		if (el.hasAttribute('data-element') && el.hasAttribute('data-disable') && el.getAttribute('data-disable') === el.getAttribute('data-element')) {
			el.addEventListener('mouseenter', function (e) {
...
// Togglable input
var inputShowAsync = function () {
	var input_show = document.querySelectorAll('.input-show > label');
	if (input_show.length) {
		Array.from(input_show).forEach(function (el) {
			"use strict";
			if (!el.classList.contains('input-show-initialized')) {
				el.classList.add('input-show-initialized');
				append_url(el, toggleText, 'toggle');
...
function openPanel(id) {
	...
	Array.from(linked).forEach(function (el) {
		...
		el.setAttribute('aria-hidden', false);
		posTabIn(el);
		/*linked_child = el.querySelectorAll('a, input, button, select, textarea, [role="button"]');
		setTimeout(function () {
			if (linked_child.length) {
				linked_child[0].focus();
			}
		}, 100);*/
	});
	//to check when element gets position sticky
...
var schemeTooltipAsync = function(){
	...
	if (scheme_tooltip.length) {
		Array.from(scheme_tooltip).forEach(function (el, in1) {
			//new_css('css-tooltips', a_css_tooltips);
...
var rangeSliderAsync = function() {
	var input_range = document.getElementsByClassName('input-range'),
		input_range_steps = document.getElementsByClassName('input-range-steps');
...
var ajaxCart = (function(module) {
	var init, formOverride, addCartItem, handleCartPanel, updateCartPanel, updateCartPage, showCartPanel, updateItemQty, handleCartUpdates, removeItem, updateCartCount, handleErrorMessage, addNote; // Define the functions
	...
	handleCartPage = function(resultsMarkup) {
		...
		if (typeof window.stripMainCartTermsCheckbox === 'function') {
			window.stripMainCartTermsCheckbox(cartPageContainer);
		}
		semanticInputAsync();
		formValidateAsync();
		inputShowAsync();
...
var productVariantsAsync = function() {
	var main_id_input = document.querySelectorAll('.m6pr select[name="id"]:not(.listening), .m6pr input[type="radio"][name="id"]:not(.listening)');
	var product_card_add_to_cart = document.querySelectorAll('.l4cl .product-card.update-variants select[name="id"]:not(.listening)');
    ...
    resultMediaItem = null,
    targetIndex = -1;

if (resultGallery && liveGallery) {
    Array.from(resultGallery.querySelectorAll('[data-media-id]')).some(function (mediaItem) {
        if (mediaItem.getAttribute('data-media-id') === targetMediaId) {
            resultMediaItem = mediaItem;
            return true;
        }
    ...
    var mainProductImage = productFormSection.querySelector('#main-product-image');

    if (mainProductImage && resultMediaItem) {
        var selectedImage = resultMediaItem.querySelector('img[data-large]') || resultMediaItem.querySelector('img'),
            selectedImageUrl = selectedImage
                ? selectedImage.getAttribute('data-large') || selectedImage.currentSrc || selectedImage.getAttribute('src')
                : '';

        if (selectedImageUrl) {
            mainProductImage.setAttribute('src', selectedImageUrl);
            mainProductImage.removeAttribute('srcset');
            mainProductImage.removeAttribute('data-src');
            mainProductImage.removeAttribute('data-srcset');
            mainProductImage.removeAttribute('sizes');
        }
    }
    ...
    if (targetIndex >= 0) {
        ...
        } else if (!mainProductImage && mediaItems[targetIndex]) {
...
var clearRalPickerFields = function (wrap) {
	if (!wrap) {
		return;
	}
	wrap.querySelectorAll('.ral-radio').forEach(function (r) {
		r.checked = false;
	});
	var ci = wrap.querySelector('.custom-ral-input');
	if (ci) {
		ci.value = '';
	}
	var sv = wrap.querySelector('.ral-selected-value');
	var sp = wrap.querySelector('.ral-selected-preview');
	if (sv) {
		sv.textContent = 'None';
	}
	if (sp) {
...
var getRalProductVariantIdControl = function (wrap) {
	var formId = wrap.getAttribute('data-form-id');
	var formEl = formId ? document.getElementById(formId) : null;
	var idControl = null;
	if (formEl && formEl.elements && formEl.elements.namedItem) {
		idControl = formEl.elements.namedItem('id');
	}
	if (!idControl && formId) {
		idControl = document.querySelector('select[name="id"][form="' + formId + '"]');
	}
	if (!idControl && formId) {
		idControl = document.querySelector('input[name="id"][form="' + formId + '"]:checked');
	}
	if (!idControl) {
		var form = wrap.closest('form');
		if (form && form.elements && form.elements.namedItem) {
			idControl = form.elements.namedItem('id');
		}
		if (!idControl && form) {
			idControl = form.querySelector('[name="id"]');
		}
	}
	return idControl;
};

var syncRalPickerVisibility = function (productRoot) {
	if (!productRoot) {
		return;
	}
	var wrap = productRoot.querySelector('.ral-picker-wrap');
	if (!wrap) {
		return;
	}
	var sectionSid = wrap.getAttribute('data-section-template');
	if (sectionSid) {
		var sectionEl = document.getElementById('shopify-section-' + sectionSid);
		if (sectionEl) {
			productRoot = sectionEl;
			wrap = productRoot.querySelector('.ral-picker-wrap');
			if (!wrap) {
				return;
			}
		}
	}
	var template = wrap.getAttribute('data-section-template');
	var finishLabel = wrap.getAttribute('data-finish-option-label');
	var ralTrigger = (wrap.getAttribute('data-ral-trigger') || 'ral').trim().toLowerCase();
	var ralIdsRaw = wrap.getAttribute('data-ral-variant-ids') || '';
	var ralIdSet = {};
	if (ralIdsRaw.trim() !== '') {
		ralIdsRaw.split(',').forEach(function (s) {
	...
	var idControl = getRalProductVariantIdControl(wrap);
	var variantId = '';
	if (idControl && typeof idControl.value !== 'undefined') {
		variantId = String(idControl.value).trim();
	}
	var showByVariantId = Object.keys(ralIdSet).length > 0 && variantId !== '' && ralIdSet[variantId] === true;
	var showByOption = false;
	var finishVal = '';
	if (template && finishLabel) {
		var normOpt = function (s) {
			return String(s || '')
				.replace(/\u00a0/g, ' ')
				.trim()
				.toLowerCase();
		};
		var optName = 'options[' + template + '-' + finishLabel + ']';
		var nameSuffix = '-' + finishLabel + ']';
		var selMatch = null;
		Array.from(productRoot.querySelectorAll('select')).forEach(function (s) {
			if (s.getAttribute('name') === optName) {
				selMatch = s;
			}
		});
		if (selMatch) {
			finishVal = normOpt(selMatch.value);
		} else {
			Array.from(productRoot.querySelectorAll('input[type="radio"]')).forEach(function (inp) {
				if (!inp.checked) {
					return;
				}
				var nm = inp.getAttribute('name') || '';
				if (nm === optName || nm.slice(-nameSuffix.length) === nameSuffix) {
					finishVal = normOpt(inp.value);
				}
			});
		}
		if (finishVal === '' && wrap.getAttribute('data-finish-option-index') !== null) {
			var idx = parseInt(wrap.getAttribute('data-finish-option-index'), 10);
			if (!isNaN(idx) && idx >= 0) {
				var formIdAttr = wrap.getAttribute('data-form-id');
				var idSel = formIdAttr
					? document.querySelector('select[name="id"][form="' + formIdAttr + '"]')
					: productRoot.querySelector('select[name="id"]');
				if (idSel && idSel.options[idSel.selectedIndex]) {
					var title = idSel.options[idSel.selectedIndex].textContent.trim();
					var parts = title.split(/\s*\/\s*/);
					if (parts[idx] !== undefined) {
						finishVal = parts[idx].trim().toLowerCase();
					}
				} else {
					var idRad = formIdAttr
						? document.querySelector('input[name="id"][form="' + formIdAttr + '"]:checked')
						: productRoot.querySelector('input[name="id"]:checked');
					if (idRad && idRad.getAttribute('title')) {
						var title2 = idRad.getAttribute('title').trim();
						var parts2 = title2.split(/\s*\/\s*/);
						if (parts2[idx] !== undefined) {
							finishVal = parts2[idx].trim().toLowerCase();
						}
					}
				}
			}
		}
		showByOption = finishVal === normOpt(ralTrigger);
	}
	var show = showByVariantId || showByOption;
	if (show) {
		wrap.classList.remove('ral-picker-wrap--hidden');
	} else {
		wrap.classList.add('ral-picker-wrap--hidden');
		clearRalPickerFields(wrap);
	}
};

window.syncRalPickerVisibility = syncRalPickerVisibility;

var initRalPickerVisibilityAll = function () {
	Array.from(document.querySelectorAll('.ral-picker-wrap')).forEach(function (wrap) {
		var t = wrap.getAttribute('data-section-template');
		var root = t ? document.getElementById('shopify-section-' + t) : null;
		if (!root) {
			root = wrap.closest('[id^="shopify-section-"]');
		}
		if (!root && t) {
			root = wrap.closest('[data-template="' + t + '"]');
		}
		if (root) {
			syncRalPickerVisibility(root);
		}
	});
};

window.initRalPickerVisibilityAll = initRalPickerVisibilityAll;

document.addEventListener('change', function (ev) {
	var el = ev.target;
	if (!el || !el.getAttribute || !el.name) {
		return;
	}
	var n = el.name;
	if (n !== 'id' && n.indexOf('options[') !== 0) {
		return;
	}
	var root = el.closest('[id^="shopify-section-"]');
	if (!root) {
		root = el.closest('[data-template]');
	}
	if (root && root.querySelector('.ral-picker-wrap')) {
		syncRalPickerVisibility(root);
	}
}, true);

var productOptionsAsync = function () {
		...
		Array.from(options_input).forEach(function (el) {
			...
			el.addEventListener('change', function() {
				setTimeout(function () {
					...
					var setUnavailable = function(el, hideData) {
						...
						if(hideData) {
							if (el.closest('.f8pr-container').querySelector('.f8pr-price')) {
								el.closest('.f8pr-container').querySelector('.f8pr-price').style.visibility = 'hidden';
							}
							if (el.closest('.f8pr-container').querySelector('.f8pr-stock')) {
								el.closest('.f8pr-container').querySelector('.f8pr-stock').style.display = 'none';
							}
							if(document.querySelector('#sticky-add-to-cart .s1pr')) {
								document.querySelector('#sticky-add-to-cart .s1pr').style.visibility = 'hidden';
							}
						}
					}
					var updateVariantInput = function() {
						...
						if (!this.currentVariant) {
							setUnavailable(productForm.querySelector('form.f8pr'), true);
							if (stickyAddToCart) {
					...
					updateVariantInput();
					var ralRoot = productForm.closest('[id^="shopify-section-"]') || productForm;
					syncRalPickerVisibility(ralRoot);
				},1);
			});
		});
	}
	initRalPickerVisibilityAll();
};
productOptionsAsync();
if (document.readyState === 'loading') {
	document.addEventListener('DOMContentLoaded', initRalPickerVisibilityAll);
} else {
	initRalPickerVisibilityAll();
}
...
if (!general.enable_cart_drawer && document.querySelectorAll('.cart-count')) {
